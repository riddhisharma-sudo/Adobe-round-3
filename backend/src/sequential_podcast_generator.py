...
backend_dir = os.path.dirname(curr_dir)  # Get backend directory
project_root = os.path.dirname(backend_dir)  # Get project root
...
VOICE_A = "onyx"
VOICE_B = "nova"
MAX_CHARS_PER_REQ = int(os.getenv("MAX_CHARS_PER_REQ", "800"))  # 0 means no chunking
...
def chunk_text(text: str, max_chars: int):
    """Naive chunker splitting on spaces when possible. If max_chars <= 0 -> no chunking."""
...
def generate_json_podcast(json_file):
    ...
    # We'll build up a list of AudioSegment pieces to concat
    final_segments = []
    ...
    for i, turn in enumerate(dialogue, start=1):
        ...
        # determine voice (fallback: alternate by index if unknown)
        if speaker.lower().startswith("host a") or speaker.lower().startswith("hosta") or speaker == "Alex":
            ...
            # unknown speaker -> alternate based on index (even/odd)
        ...
        for ci, chunk in enumerate(chunks, start=1):
            ...
            # small throttle to be nice to the service
            sleep(0.15)

        # add silence between turns (once per turn)
        if PAUSE_BETWEEN_TURNS_MS > 0:
            final_segments.append(AudioSegment.silent(duration=PAUSE_BETWEEN_TURNS_MS))

    # Concatenate everything into single AudioSegment
